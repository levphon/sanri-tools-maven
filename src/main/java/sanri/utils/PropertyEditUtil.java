	...
	static{
		DEFAULT_PRIMITIVE_HANDLER = new PrimitiveHandler() {
			...
			public Object handler(String name, Object dest, Object source) {
				...
				if(clazz == int.class){
					int sourceValue = NumberUtil.toInt(sourceValue_);
					if(sourceValue == 0 ){
						return dest;
					}
				}else if(clazz == long.class){
					long sourceValue = NumberUtil.toLong(sourceValue_);
					if(sourceValue == 0 ){
						return dest;
					}
				}else if(clazz == double.class){
					double sourceValue = NumberUtil.toDouble(sourceValue_);
					if(sourceValue == 0 ){
						return dest;
					}
				}else if(clazz == float.class){
					float sourceValue = NumberUtil.toFloat(sourceValue_);
					if(sourceValue == 0 ){
						return dest;
					}
				}
